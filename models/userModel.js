  ...
  return new Promise(resolve => {
    try {
      ...
      ConnectDB.acquire((err, conn) => {
        ...
        const sql = `INSERT INTO ${USER_TABLE} (username, password, auth) values ('${user.username}', '${user.password}', '${user.auth}') `;
        conn.query(sql, (err, result) => {
          conn.release();
...
exports.storeToken = async function(tokenObject) {
  ...
  try {
      ...
      // console.log(isExist);
      if (!isExist.error) {
        sql = isExist.data.length ? `UPDATE token SET token = '${token}', refresh_token = '${refreshToken}' WHERE user = '${userID}'` : `INSERT INTO token (token, refresh_token, user) VALUES ('${token}', '${refreshToken}', '${userID}')`;
        return queryMySQL(sql);
      } else {
        // error handle

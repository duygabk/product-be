...
exports.userSignup = async function(req, res, next) {
  ...
  const isExist =
    allUser && allUser.data.length
      ? allUser.data.some(user => user.username === username)
      : false;
  // console.log(isExist);

  if (isExist) {
    // Already Exist => send error to Client
    res.json({ error: true, message: `${username} is already exist!!!` });
  } else {
    // add new user and send message to Client
    // hash password
    // password = await encryptPassword(password);
    // console.log({password});
    const added = await userModel.addNewUser({
      username,
      password,
      auth: "member"
    });
...
exports.userLogin = async function(req, res, next) {
  ...
  if (!isExistUser.error) {
    ...
    const saveToDB = await userModel
      .storeToken({ token, refreshToken, userID })
      .then(res => res)
      .catch(err => err);
...
exports.refreshToken = async function(req, res) {
  ...
  if (error) {
    ...
    const response =
      data && data.length
        ? { error: false, data: data[0] }
        : { error: true, message: "Refresh Token not macth" };

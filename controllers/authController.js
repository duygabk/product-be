...
let isAuth = async (req, res, next) => {
  const tokenFromClient = req.headers["authorization"] || req.headers["Authorization"];
  // const tokenFromClient = req.headers;
  console.log({tokenFromClient});
  console.log(req.path);
  if (tokenFromClient) {
    try {
      const decoded = await jwtHelper.verifyToken(tokenFromClient).then(res => res).catch(err => err);
      // console.log(decoded);
      // OK
...
const adminAutho = async (req, res, next) => {
  ...
  const tokenFromClient = req.headers["authorization"] || req.headers["Authorization"] || req.headers["x-token"];
  console.log({tokenFromClient});
  if (tokenFromClient) {
